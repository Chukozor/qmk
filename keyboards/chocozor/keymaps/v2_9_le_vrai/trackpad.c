...
static float acceleration_factor(int16_t, int16_t);
...
static inline void xy_filter_apply(report_mouse_t *r) {
    ...
    if (mag <= MAXIMUM_ACCELERATION_ZONE) {
        float factor = acceleration_factor(r->x, r->y);
        xy_accum_x += (float)r->x * factor;
        xy_accum_y += (float)r->y * factor;

        int16_t out_x = (int16_t)xy_accum_x;
        int16_t out_y = (int16_t)xy_accum_y;
...
static float acceleration_factor(int16_t x, int16_t y){
